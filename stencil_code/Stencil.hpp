// DANGER DANGER this is evolving from a  clone of Map into a Stencil
// could be chaotic
    ...
    public:
        ...
        class StencilImplementation {
	        ...
	        size_t width;      // NEW
                ...
                void threadStencil(ThreadArgument<IN,OUT> *threadArguments, size_t width, size_t threadID, ARGs... args) {  //NEW
			...
			size_t inputSize = input->size();
				...
				IN *neighbourhood = (IN *) malloc(sizeof(IN)*width);   
				// Hardwired for wrap-around boundaries just now
   				      ...
   				      neighbourhood[i] = input->at((elementIndex-width+i+inputSize)%inputSize);  // possibly :)
                                    }
	  ...
	  StencilImplementation(Elemental<EL> elemental, size_t width, size_t threads) : elemental(elemental), width(width), nthreads(threads){ 
		  this->nDataBlocks = NDATABLOCKS; 
            ...
            public:
		      ...
		      //output[0] = elemental.elemental( input[0], args...);
                    ...
                    for(size_t t=0; t< nthreads; ++t ){
		      THREADS[t] = new std::thread(&StencilImplementation<EL>::threadStencil<IN,OUT,ARGs...>, this, threadArguments, width, t, args...); 
                    }
